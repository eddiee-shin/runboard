...
import { extractGarminActivityUrl, getGarminImportUrl, mergeGarminClipboardText, parseGarminActivityUrl, parseGarminEmbedHtml } from '../src/lib/garmin-link.ts'
...
test('reads an encoded Garmin activity URL from the upload query', () => {
  const url = `https://connect.garmin.com/modern/activity/${id}`
  assert.equal(getGarminImportUrl(`?url=${encodeURIComponent(url)}`), url)
  assert.equal(getGarminImportUrl(`?mode=garmin-link&url=${encodeURIComponent(`Check out my run ${url}`)}`), url)
  assert.equal(getGarminImportUrl('?url=https%3A%2F%2Fexample.com%2Factivity%2F123'), null)
})
